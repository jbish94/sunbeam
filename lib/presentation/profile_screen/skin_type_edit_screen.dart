...
class SkinTypeEditScreen extends StatefulWidget {
  ...
  const SkinTypeEditScreen({
    Key? key,
    required this.currentSkinType,
...
class _SkinTypeEditScreenState extends State<SkinTypeEditScreen> {
  ...
  void initState() {
    super.initState();
    // Extract the Roman numeral from the current skin type
    if (widget.currentSkinType != null &&
        widget.currentSkinType!.contains('Type ')) {
      final parts = widget.currentSkinType!.split(' ');
      if (parts.length > 1) {
        _selectedSkinType = parts[1];
      }
    }
  }

  void _updateSkinType(String skinType) {
    setState(() {
  ...
  void _saveSkinType() {
    if (_selectedSkinType == null) {
      _showToast('Please select a skin type');
      return;
    }

    final skinTypeData = _skinTypes.firstWhere(
      (type) => type['type'] == _selectedSkinType,
    );

    final formattedSkinType =
        'Type $_selectedSkinType - ${skinTypeData['description']}';
    widget.onSkinTypeChanged(formattedSkinType);
    _showToast('Skin type updated successfully');
  ...
  Widget build(BuildContext context) {
    final bool hasSelection = _selectedSkinType != null;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          TextButton(
            onPressed: hasSelection ? _saveSkinType : null,
            child: Text(
              'Save',
  ...
  Widget _buildSkinTypeCard(Map<String, dynamic> skinType) {
    final bool isSelected = _selectedSkinType == skinType['type'];

    return Material(
      color: Colors.transparent,
      child: InkWell(
        onTap: () => _updateSkinType(skinType['type']),
